...
var paper = new function() {
    ...
    var Base = new function() {
        ...
        } : proto.hasOwnProperty, toString = proto.toString, proto = Array.prototype, isArray = Array.isArray = Array.isArray ||
        function(obj) {
          return toString.call(obj) === '[object Array]';
        }, slice = proto.slice, forEach = proto.forEach = proto.forEach ||
        function(iter, bind) {
        ...
        }, _define = Object.defineProperty, _describe = Object.getOwnPropertyDescriptor;
        ...
        function inject(dest, src, enumerable, base, preserve, generics) {
          ...
          function field(name, val, dontCheck, generics) {
            ...
            if ((dontCheck || val !== undefined && has.call(src, name)) && (!preserve || !prev)) {
              ...
              if (!res || func || !res.get && !res.set) res = {
                value: res,
                writable: true
              };
    ...
    var PaperScope = this.PaperScope = Base.extend({
      ...
      setup: function(canvas) {
        ...
        if (canvas) this.view = new View(canvas);
      },
    ...
    var Point = this.Point = Base.extend({
      ...
      rotate: function(angle, center) {
        ...
        var point = center ? this.subtract(center) : this,
          ...
          c = Math.cos(angle);
        point = Point.create(
        point.x * c - point.y * s, point.y * c + point.x * s);
    ...
    var Size = this.Size = Base.extend({
      ...
      statics: {
        ...
        min: function(size1, size2) {
          return Size.create(
          Math.min(size1.width, size2.width), Math.min(size1.height, size2.height));
        },

        max: function(size1, size2) {
          return Size.create(
          Math.max(size1.width, size2.width), Math.max(size1.height, size2.height));
        },
    ...
    var Matrix = this.Matrix = Base.extend({
      ...
      toString: function() {
        var format = Base.formatNumber;
        return '[[' + [format(this._a), format(this._b), format(this._tx)].join(', ') + '], [' + [format(this._c), format(this._d), format(this._ty)].join(', ') + ']]';
      },
      ...
      _transformPoint: function(point, dest, dontNotify) {
        ...
        return dest.set(
        x * this._a + y * this._b + this._tx, x * this._c + y * this._d + this._ty, dontNotify);
      },
      ...
      createInverse: function() {
        ...
        this._d / det, -this._c / det, -this._b / det, this._a / det, (this._b * this._ty - this._d * this._tx) / det, (this._c * this._tx - this._a * this._ty) / det);
      },
    ...
    var Item = this.Item = Base.extend({
      ...
      hitTest: function(point, options, matrix) {
        options = HitResult.getOptions(point, options);
        point = options.point;
        if (!this._children && !this.getRoughBounds(matrix).expand(options.tolerance)._containsPoint(point)) return null;
        if ((options.center || options.bounds) && !(this instanceof Layer && !this._parent)) {
          var bounds = this.getBounds(),
            that = this,
            points = ['TopLeft', 'TopRight', 'BottomLeft', 'BottomRight', 'LeftCenter', 'TopCenter', 'RightCenter', 'BottomCenter'],
            res;
      ...
      _createBounds: function(rect) {
        return LinkedRectangle.create(this, 'setBounds', rect.x, rect.y, rect.width, rect.height);
      },
      ...
      setBounds: function(rect) {
        ...
        if (rect.width != bounds.width || rect.height != bounds.height) {
          matrix.scale(
          bounds.width != 0 ? rect.width / bounds.width : 1, bounds.height != 0 ? rect.height / bounds.height : 1);
        }
      ...
      scale: function(hor, ver, center) {
        ...
        return this.transform(new Matrix().scale(hor, ver, center || this.getPosition()));
      },
      ...
      rotate: function(angle, center) {
        return this.transform(new Matrix().rotate(angle, center || this.getPosition()));
      },
      ...
      shear: function(hor, ver, center) {
        ...
        return this.transform(new Matrix().shear(hor, ver, center || this.getPosition()));
      },
      ...
      statics: {
        ...
        draw: function(item, ctx, param) {
          ...
          item.draw(ctx, param);
          if (itemOffset) param.offset = savedOffset;
          ...
          if (tempCanvas) {
            ...
            if (item._blendMode !== 'normal') {
              ...
              BlendMode.process(item._blendMode, ctx, parentCtx, item._opacity, pixelOffset);
            } else {
              parentCtx.save();
      ...
      function removeAll(set) {
        for (var id in set) {
          ...
          for (var type in sets) {
            var other = sets[type];
            if (other != set && other[item.getId()]) delete other[item.getId()];
          }
        }
      }
      ...
      function installHandler(name) {
        ...
        if (!func || !func._installed) {
          ...
          hash[handler] = function(event) {
            if (name === 'up') sets.drag = {};
            removeAll(sets[name]);
            sets[name] = {};
            if (this.base) this.base(event);
          };
        ...
        removeOn: function(obj) {
          for (var name in obj) {
            if (obj[name]) {
              ...
              if (name === 'drag') installHandler('up');
              installHandler(name);
            }
          }
          return this;
    ...
    var Group = this.Group = Item.extend({
      ...
      draw: function(ctx, param) {
        ...
        if (clipItem) Item.draw(clipItem, ctx, param);
        for (var i = 0, l = this._children.length; i < l; i++) {
    ...
    var PlacedItem = this.PlacedItem = Item.extend({
      ...
      getBounds: function() {
        ...
        if (useCache && this._bounds) return this._bounds;
        var bounds = this.getStrokeBounds(arguments[0]);
        if (useCache) bounds = this._bounds = this._createBounds(bounds);
        return bounds;
      },
    ...
    var Raster = this.Raster = PlacedItem.extend({
      ...
      getAverageColor: function(object) {
        if (!object) object = this.getBounds();
        var bounds, path;
    ...
    var PlacedSymbol = this.PlacedSymbol = PlacedItem.extend({
      ...
      draw: function(ctx, param) {
        if (param.selection) {
          Item.drawSelectedBounds(this.symbol._definition.getStrokeBounds(), ctx, this._matrix);
        } else {
          ctx.save();
          this._matrix.applyToContext(ctx);
      ...
      getCrossings: function(point, matrix, roots) {
        ...
        for (var i = 0; i < num; i++) {
          ...
          if (t >= 0 && t < 1 && Curve.evaluate(vals, t, 0).x > point.x) {
            ...
            this.getPrevious().getValues(matrix), 1, 1).y * Curve.evaluate(vals, t, 1).y >= 0) continue;
            crossings++;
          }
        }
        return crossings;
    ...
    var Path = this.Path = PathItem.extend({
      ...
      getCurves: function() {
        if (!this._curves) {
          ...
          this._curves[i] = Curve.create(this, segments[i], segments[i + 1] || segments[0]);
        }
        return this._curves;
      },
      ...
      contains: function(point, matrix) {
        point = Point.read(arguments);
        if (!this._closed || !this.getRoughBounds(matrix)._containsPoint(point)) return false;
        var curves = this.getCurves(),
          crossings = 0,
          roots = [];
        for (var i = 0, l = curves.length; i < l; i++)
      ...
      _hitTest: function(point, options, matrix) {
        ...
        function checkSegment(segment, ends) {
          ...
          for (var j = ends || options.segments ? 0 : 2, m = !ends && options.handles ? 6 : 2; j < m; j += 2) {
            if (point.getDistance(coords[j], coords[j + 1]) < tolerance) return new HitResult(j == 0 ? 'segment' : 'handle-' + (j == 2 ? 'in' : 'out'), that, {
              segment: segment
            });
          }
        }
        ...
        if (options.stroke && radius > 0) loc = this.getNearestLocation(point, matrix);
        if (!(loc && loc._distance <= radius) && options.fill && this.getFillColor() && this.contains(point, matrix)) return new HitResult('fill', this);
        if (!loc && options.stroke && radius > 0) loc = this.getNearestLocation(point, matrix);
      ...
      function drawHandles(ctx, segments) {
        for (var i = 0, l = segments.length; i < l; i++) {
          ...
          if (selected || (state & SelectionState.HANDLE_IN)) drawHandle(ctx, point, segment._handleIn);
          if (selected || (state & SelectionState.HANDLE_OUT)) drawHandle(ctx, point, segment._handleOut);
          ctx.save();
          ctx.beginPath();
        ...
        smooth: function() {
          ...
          for (var i = overlap; i <= n - overlap; i++) {
            ...
            if (i < n) {
              ...
              if (i < n - 1) handleIn = new Point(
              2 * knots[i + 1]._x - x[i + 1], 2 * knots[i + 1]._y - y[i + 1]);
              else handleIn = new Point(
      ...
      return {
        ...
        quadraticCurveTo: function(handle, to) {
          ...
          this.cubicCurveTo(
          handle.add(current.subtract(handle).multiply(1 / 3)), handle.add(to.subtract(handle).multiply(1 / 3)), to);
        },
      ...
      function getBounds(that, matrix, strokePadding) {
        ...
        if (that._closed) processSegment(first);
        return Rectangle.create(min[0], min[1], max[0] - min[0], max[1] - min[1]);
      }
      ...
      return {
        getBounds: function() {
          ...
          if (useCache) this._bounds = bounds;
          return bounds;
        },
        ...
        getStrokeBounds: function() {
          ...
          var useCache = arguments[0] === undefined;
          if (useCache && this._strokeBounds) return this._strokeBounds;
          ...
          function addJoin(segment, join) {
            if (join === 'round' || !segment._handleIn.isZero() && !segment._handleOut.isZero()) {
              ...
              var curve2 = segment.getCurve(),
                ...
                line1 = new Line(point.subtract(normal1), new Point(-normal1.y, normal1.x)),
                line2 = new Line(point.subtract(normal2), new Point(-normal2.y, normal2.x)),
          ...
          function addCap(segment, cap, t) {
            ...
            case 'square':
              var curve = segment.getCurve(),
                ...
                normal = curve.getNormal(t).normalize(radius);
              if (cap === 'square') point = point.add(normal.y, -normal.x);
              add(point.add(normal));
              add(point.subtract(normal));
              break;
            }
          ...
          if (useCache) this._strokeBounds = bounds;
          return bounds;
        },
        ...
        getHandleBounds: function() {
          var matrix = arguments[0],
            useCache = matrix === undefined;
          if (useCache && this._handleBounds) return this._handleBounds;
          var coords = new Array(6),
          ...
          var bounds = Rectangle.create(x1, y1, x2 - x1, y2 - y1);
          if (useCache) this._handleBounds = bounds;
          return bounds;
        },

        getRoughBounds: function() {
          var useCache = arguments[0] === undefined;
          if (useCache && this._roughBounds) return this._roughBounds;
          var bounds = this.getHandleBounds(arguments[0], this.strokeWidth, this.getStrokeJoin() == 'miter' ? this.strokeWidth * this.getMiterLimit() : this.strokeWidth);
          if (useCache) this._roughBounds = bounds;
          return bounds;
        }
      };
    });
    ...
    Path.inject({
      statics: new function() {
        ...
        var ovalSegments = [
        new Segment([0, 0.5], [0, kappa], [0, -kappa]), new Segment([0.5, 0], [-kappa, 0], [kappa, 0]), new Segment([1, 0.5], [0, -kappa], [0, kappa]), new Segment([0.5, 1], [kappa, 0], [-kappa, 0])];

        return {
          Line: function() {
            var step = Math.floor(arguments.length / 2);
            return new Path(
            Segment.read(arguments, 0, step), Segment.read(arguments, step, step));
          },
          ...
          Rectangle: function(rect) {
            ...
            var left = rect.x,
              ...
              path = new Path();
            path._add([
            new Segment(Point.create(left, bottom)), new Segment(Point.create(left, top)), new Segment(Point.create(right, top)), new Segment(Point.create(right, bottom))]);
            path._closed = true;
            return path;
          },
          ...
          RoundRectangle: function(rect, size) {
            ...
            path._add([
            new Segment(bl.add(size.width, 0), null, [-uSize.width, 0]), new Segment(bl.subtract(0, size.height), [0, uSize.height], null),

            new Segment(tl.add(0, size.height), null, [0, -uSize.height]), new Segment(tl.add(size.width, 0), [-uSize.width, 0], null),

            new Segment(tr.subtract(size.width, 0), null, [uSize.width, 0]), new Segment(tr.add(0, size.height), [0, -uSize.height], null),

            new Segment(br.subtract(0, size.height), null, [0, uSize.height]), new Segment(br.subtract(size.width, 0), [uSize.width, 0], null)]);
            path._closed = true;
            return path;
          },
          ...
          Oval: function(rect) {
            ...
            for (var i = 0; i < 4; i++) {
              ...
              segment._point.multiply(size).add(point), segment._handleIn.multiply(size), segment._handleOut.multiply(size));
            }
            path._add(segments);
            path._closed = true;
            return path;
    ...
    var CompoundPath = this.CompoundPath = PathItem.extend({
      ...
      Base.each(['lineTo', 'cubicCurveTo', 'quadraticCurveTo', 'curveTo', 'arcTo', 'lineBy', 'curveBy', 'arcBy'], function(key) {
        fields[key] = function() {
    ...
    var PathFitter = Base.extend({
      ...
      fit: function() {
        ...
        this.fitCubic(0, this.points.length - 1, this.points[1].subtract(this.points[0]).normalize(), this.points[this.points.length - 2].subtract(
      ...
      toCssString: function() {
        if (!this._cssString) {
          var color = this.convert('rgb'),
            alpha = color.getAlpha(),
            components = [
            Math.round(color._red * 255), Math.round(color._green * 255), Math.round(color._blue * 255), alpha != null ? alpha : 1];
    ...
    var DomEvent = {
      ...
      getPoint: function(event) {
        ...
        return Point.create(
        pos.pageX || pos.clientX + document.documentElement.scrollLeft, pos.pageY || pos.clientY + document.documentElement.scrollTop);
      },
    ...
    var View = this.View = PaperScopeItem.extend({
        ...
        if (canvas instanceof HTMLCanvasElement) {
          ...
          if (PaperScript.hasAttribute(canvas, 'resize')) {
            var offset = DomElement.getOffset(canvas, true),
              that = this;
            size = DomElement.getViewportBounds(canvas).getSize().subtract(offset);
            canvas.width = size.width;
            canvas.height = size.height;
            DomEvent.add(window, {
              resize: function(event) {
                if (!DomElement.isInvisible(canvas)) offset = DomElement.getOffset(canvas, true);
                that.setViewSize(DomElement.getViewportBounds(canvas).getSize().subtract(offset));
              }
            });
          } else {
            size = DomElement.isInvisible(canvas) ? Size.create(parseInt(canvas.getAttribute('width')), parseInt(canvas.getAttribute('height'))) : DomElement.getSize(canvas);
          }
          ...
          if (size.isZero()) size = new Size(1024, 768);
      ...
      getBounds: function() {
        if (!this._bounds) this._bounds = this._getInverse()._transformBounds(
      ...
      setOnFrame: function(onFrame) {
        ...
        this._onFrameCallback = function(param, dontRequest) {
          requested = false;
          if (!that._onFrame) return;
          paper = that._scope;
          requested = true;
          if (!dontRequest) {
            DomEvent.requestAnimationFrame(that._onFrameCallback, that._canvas);
          }
      ...
      var tool, timer, curPoint, tempFocus, dragging = false;
      ...
      function mousemove(event) {
        ...
        if (!(view = view || View._focused) || !(tool = view._scope.tool)) return;
      ...
      function mouseup(event) {
        ...
        curPoint = null;
        if (tool) {
          if (timer != null) timer = clearInterval(timer);
      ...
      return {
        _createEvents: function() {
          ...
          function mousedown(event) {
            View._focused = view;
            if (!(tool = view._scope.tool)) return;
            curPoint = viewToProject(view, event);
            if (tool.onHandleEvent('mousedown', curPoint, event)) view.draw(true);
            if (tool.eventInterval != null) timer = setInterval(mousemove, tool.eventInterval);
            dragging = true;
          }
    ...
    var Key = this.Key = new function() {
        ...
        function handleKey(down, keyCode, charCode, event) {
          ...
          if (tool && tool[handler]) {
            ...
            if (tool[handler](keyEvent) === false) keyEvent.preventDefault();
    ...
    var Tool = this.Tool = PaperScopeItem.extend({
      ...
      updateEvent: function(type, pt, minDistance, maxDistance, start, needsChange, matchMaxDistance) {
        if (!start) {
      ...
      onHandleEvent: function(type, pt, event) {
        ...
        case 'mousedrag':
          ...
          while (this.updateEvent(type, pt, this.minDistance, this.maxDistance, false, needsChange, matchMaxDistance)) {
            if (this.onMouseDrag) {
          ...
          break;
        case 'mouseup':
          if ((this._point.x != pt.x || this._point.y != pt.y) && this.updateEvent('mousedrag', pt, this.minDistance, this.maxDistance, false, false, false)) {
            if (this.onMouseDrag) {
              this.onMouseDrag(new ToolEvent(this, type, event));
              called = true;
            }
          }
          this.updateEvent(type, pt, null, this.maxDistance, false, false, false);
          if (this.onMouseUp) {
          ...
          break;
        case 'mousemove':
          while (this.updateEvent(type, pt, this.minDistance, this.maxDistance, this._firstMove, true, false)) {
            if (this.onMouseMove) {
    ...
    var BlendMode = {
      process: function(blendMode, srcContext, dstContext, alpha, offset) {
        var srcCanvas = srcContext.canvas,
          ...
          max = Math.max,
          abs = Math.abs,
          sr, sg, sb, sa, br, bg, bb, ba, dr, dg, db;
    ...
    var PaperScript = this.PaperScript = new function() {
        ...
        function handleOperator(operator, left, right) {
          ...
          if (operators[operator] && isDynamic(left)) {
            ...
            return ['call', ['name', '$eval'], [left, ['string', operator], right]];
          }
        }
